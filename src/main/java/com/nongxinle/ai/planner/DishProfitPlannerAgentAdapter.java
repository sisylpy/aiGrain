...
/**
 * C-26：菜品毛利只读 {@link PlannerAgentAdapter}；{@code targetAgent} / {@code targetTool} 与
 * {@link com.nongxinle.ai.agent.business.BusinessAgentNames#DISH_PROFIT_ANALYSIS} /
 * {@link AiBusinessToolIds#DISH_PROFIT_ANALYSIS} 对齐。
 *
 * <p>无 {@link DishProfitPlannerReadBridge} 时诚实 {@code DEGRADED}。{@link DishProfitPlannerRealReadBridge} 走
 * {@code readWithExecutionContext}；Spring Bean 下调用 {@link com.nongxinle.ai.graph.business.DishProfitQueryToolExecutor}（见 C-29 Hydrated case）。</p>
 */
public final class DishProfitPlannerAgentAdapter implements PlannerAgentAdapter {

    public static final String TARGET_AGENT = BusinessAgentNames.DISH_PROFIT_ANALYSIS;
    public static final String TARGET_TOOL = AiBusinessToolIds.DISH_PROFIT_ANALYSIS;
    ...
    private final DishProfitPlannerReadBridge readBridge;

    public DishProfitPlannerAgentAdapter() {
        this(null);
    }

    public DishProfitPlannerAgentAdapter(DishProfitPlannerReadBridge readBridge) {
    ...
    public PlannerStepExecutionResponse invoke(PlannerAgentAdapterRequest request) {
        if (readBridge == null) {
            return markerDegraded(MARKER_NO_REAL_CONTEXT, "read_bridge_null:dish_profit_read_not_invoked");
        }
        DishProfitPlannerReadRequest readReq = mergeReadRequest(request);
        ...
        DishProfitPlannerReadResponse out;
        if (readBridge instanceof DishProfitPlannerRealReadBridge real) {
            DishProfitPlannerExecutionContext ctx = request.getDishProfitExecutionContext();
            if (ctx == null) {
                out =
                        DishProfitPlannerReadResponse.builder()
                                .status(DishProfitPlannerReadStatus.DEGRADED)
                                .errorCode(DishProfitPlannerRealReadBridge.ERROR_NO_PLANNER_EXECUTION_CONTEXT_ON_REQUEST)
                                .errorMessage(
                                        "missing DishProfitPlannerExecutionContext on PlannerAgentAdapterRequest")
                                .build();
            } else {
                DishProfitPlannerExecutionContext merged =
            ...
            out = readBridge.readDishProfit(readReq);
        }
        return mapReadResponse(out);
    }

    private static DishProfitPlannerReadRequest mergeReadRequest(PlannerAgentAdapterRequest request) {
        if (request == null) {
            return DishProfitPlannerReadRequest.builder().build();
        }
        DishProfitPlannerReadRequest nested = request.getDishProfitReadRequest();
        String ref = coalesce(
    ...
    private static boolean hasResolvableScope(DishProfitPlannerReadRequest r) {
        ...
        if (r.getVisibleStores() != null) {
            for (DishProfitPlannerVisibleStore s : r.getVisibleStores()) {
    ...
    private static PlannerStepExecutionResponse mapReadResponse(DishProfitPlannerReadResponse resp) {
        if (resp == null) {
            return markerDegraded(MARKER_NO_REAL_CONTEXT, "bridge_returned_null:dish_profit_read_not_invoked");
        }
        DishProfitPlannerReadStatus s =
                resp.getStatus() != null ? resp.getStatus() : DishProfitPlannerReadStatus.DEGRADED;
        if (s == DishProfitPlannerReadStatus.OK) {
        ...
        if (s == DishProfitPlannerReadStatus.FAILED) {
            return PlannerStepExecutionResponse.builder()
        ...
        String code = !isBlank(resp.getErrorCode()) ? resp.getErrorCode().trim() : "dish_profit_read_degraded";
