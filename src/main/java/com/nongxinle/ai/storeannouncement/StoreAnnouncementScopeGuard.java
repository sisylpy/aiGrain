...
/**
 * 店内公告门店成员范围校验：仅确认「当前用户属于该集团/门店 subtree」。
 * 不校验 WORK_RECORD 私人归属，不要求 AI 对话或工作记录权限。
 */
@Component
@RequiredArgsConstructor
public class StoreAnnouncementScopeGuard {

    private final GbDepartmentUserService departmentUserService;
    private final GbDepartmentMapper departmentMapper;

    public ResolvedScope resolveStoreMember(Long userId, Long departmentId, Long distributerId) {
        if (userId == null) {
