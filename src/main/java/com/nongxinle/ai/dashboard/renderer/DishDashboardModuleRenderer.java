...
public class DishDashboardModuleRenderer implements AiDashboardModuleRenderer {

    private final GbDepFoodBusinessInsightService insightService;
    ...
    public AiDashboardModuleItem render(RenderContext ctx, GbAiDashboardModuleEntity module) {
        try {
            ...
            Map<String, Object> insight = insightService.buildInsight(disId, depFatherId, startDate, endDate);

            Map<String, Object> payload = new LinkedHashMap<>();
            payload.put("businessInsightSummary", insight.get("businessInsightSummary"));
            payload.put("scopeOutboundSubtotals", insight.get("scopeOutboundSubtotals"));

            return buildModule(module, "SUCCESS", null, payload);
        } catch (Exception e) {
            log.error("菜品看板渲染失败", e);
            return buildModule(module, "FAILED", "菜品看板数据获取失败: " + e.getMessage(), new LinkedHashMap<>());
        }
    }
