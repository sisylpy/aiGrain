...
1. **当前契约**请以以下文档及代码为准（按优先级）：
   ...
   - `docs/ai/d1x-rankingtype-and-duplicate-responsibility-inventory.md`（**现网契约**）
   - `docs/ai/d1x-v2-only-time-source-cleanup-inventory.md`（V2-only 时间主链）
   - `docs/ai/phase2-tool-request-sql-input-plan.md`
   - `docs/API_INTEGRATION.md`
   - `docs/PERMISSION_MODEL.md`
   - `src/main/resources/PROJECT_AGENT_ARCHITECTURE.md`
...
2. **Replay 断言优先级（现网）**
   ...
   - **历史 JSON** 中若出现已删除 Tool id（如 `purchase_query`、`stock_query`、`dish_sales_query`、`gross_margin_calculator`、`business_overview_query`），视为 **Historical raw capture**；说明见 `docs/legacy-reference/*-removed.md`。**禁止**据此恢复 Tool、Composer raw fallback 或 classic business overview 主链。
