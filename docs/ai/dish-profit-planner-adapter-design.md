...
| caseId | 状态 |
|--------|------|
| **`PLANNER_EXECUTOR_DISH_PROFIT_ADAPTER_CORE`** | **C-26** — 无 Bridge；**`step_dish_profit_adapter`** `DEGRADED`；**`plannerDishProfitAdapterHonesty=ADAPTER_NO_REAL_CONTEXT`** |
...
| **`PLANNER_EXECUTOR_DISH_PROFIT_ADAPTER_REAL_BRIDGE_CORE`** | **C-27** — `AiPlannerExecutorDishProfitAdapterRealBridgeGraphCase` + Harness 内 **`new DishProfitPlannerRealReadBridge()`**；默认不 Hydrate **`AiRunState`/`AiResolvedQueryContext`** → 步 **`step_dish_profit_adapter_real`** **`DEGRADED`**（如 **`ADAPTER_NO_RUN_STATE:run_state_ref_not_hydrated`**）；**不**调 Tool；**`plannerDishProfitAdapterHonesty=REAL_BRIDGE_HARNESS_INCOMPLETE_CONTEXT`** |
