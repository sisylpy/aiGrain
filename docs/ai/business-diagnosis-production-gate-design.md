...
| `BUSINESS_OVERVIEW` | `business_overview_path` | `business_overview_summary` | `BUSINESS_OVERVIEW` | `business_overview_path` | **允许当且仅当** §3.3.3a **四域 orchestration** | 四域专线 **MULTI_AGENT** 或与 **`isStructuredBusinessOverviewFourDomainOrchestrationSurface`** 一致 | 不满足则 **现有** **`DEFAULT_BUSINESS_OVERVIEW_TOOLS`** 概览（含看板链路） |
...
| `BUSINESS_OVERVIEW` | `business_overview_path` | （**非** §3.3.3a，且 **非** 允许 wire） | 同左 | 同左 | **禁止** | **legacy 经营概览**（**`AiBusinessToolIds.DEFAULT_BUSINESS_OVERVIEW_TOOLS`**）**不含** Composite 六步 | 现有 **经营概览** DataPlanner 分支 |
