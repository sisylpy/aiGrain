# PlannerExecutor v1 — Harness 组合执行基础设施

> **读者**：餐饮经营 AI Harness 工程师。  
> **现网**：**`PlannerExecutor`**、**`PlannerAgentAdapterStepExecutor`**、**`PlannerExecutionPlan`** / **`StepResult`**、四域 **`*PlannerAgentAdapter`** + **`*RealReadBridge`**、**`BusinessDiagnosisCompositeExecutionService`** 均已落地。  
> **契约**：可复现多步读链路、**`degradedSteps`**、**`usedTools`**、trace 与 Replay 字段；与 **ROUTED_AGENT** / **MULTI_AGENT** 生产主链分层（见 `docs/ai/harness-composer-architecture.md`）。  
> **Composite 主验收**：**`PLANNER_EXECUTOR_BUSINESS_DIAGNOSIS_COMPOSITE_ALL_REAL_CORE`**（C-35）、**`…_GROUP_CORE`**（C-48）、**`…_STOCK_DEGRADED_CORE`**（C-42）。单域 Adapter Harness case 已移除（P1-B Final）。  
> **下一阶段**：老板常问业务能力（D-1）— **`[next-business-capability-roadmap.md](./next-business-capability-roadmap.md)`**；**C-66** dashboard **暂缓**。

**P1-B Harness 验收分层（P1-B Final 已落地）**：

| 分层 | caseId |
|------|--------|
| **当前主验收** | **`PLANNER_EXECUTOR_BUSINESS_DIAGNOSIS_COMPOSITE_ALL_REAL_CORE`**（C-35）、**`PLANNER_EXECUTOR_BUSINESS_DIAGNOSIS_COMPOSITE_GROUP_CORE`**（C-48）、**`PLANNER_EXECUTOR_BUSINESS_DIAGNOSIS_COMPOSITE_STOCK_DEGRADED_CORE`**（C-42）、**`DISH_PROFIT_MATRIX_P1`**；辅以 **`PLANNER_EXECUTOR_MOCK_CORE`** / **`MOCK_DEGRADED_CORE`** |
| **Removed（P1-B2a）** | 四域 **`*_ADAPTER_CORE`**、**`*_ADAPTER_REAL_BRIDGE_CORE`**（8 个负例） |
| **Removed（P1-B Final）** | 四域 **`*_FAKE_OK_CORE`**、**`*_REAL_BRIDGE_HYDRATED_CORE`**、**`*_GROUP_HYDRATED_CORE`**（12 个单域 Adapter）；**`PLANNER_EXECUTOR_BUSINESS_DIAGNOSIS_COMPOSITE_CORE`**（C-31 全 MOCK）；**`Fake*PlannerReadBridge`**；Hydrated 物化迁至 **`PlannerCompositeHarnessContext`** |
| **说明** | 单域 Adapter 调试能力已由 **Composite strict** 取代；勿再 curl 已删 caseId |

详见 **`[business-diagnosis-composite-plan-design.md](./business-diagnosis-composite-plan-design.md) §2**、**本文 §27**。
...
**CaseId（C-21 出库/核销 Adapter 骨架）**：`PLANNER_EXECUTOR_STOCK_REDUCE_ADAPTER_CORE` — **Removed（P1-B2a）**；GraphCase 已删，替代：`PLANNER_EXECUTOR_STOCK_REDUCE_ADAPTER_FAKE_OK_CORE` / HYDRATED / GROUP / Composite strict。
...
**CaseId（C-26 菜品毛利 Adapter 骨架）**：`PLANNER_EXECUTOR_DISH_PROFIT_ADAPTER_CORE` — **Removed（P1-B2a）**；GraphCase 已删，替代：`PLANNER_EXECUTOR_DISH_PROFIT_ADAPTER_FAKE_OK_CORE` / HYDRATED / GROUP / Composite strict。
...
**CaseId（C-27 菜品毛利 RealBridge 骨架）**：`PLANNER_EXECUTOR_DISH_PROFIT_ADAPTER_REAL_BRIDGE_CORE` — **Removed（P1-B2a）**；GraphCase 已删，替代：`PLANNER_EXECUTOR_DISH_PROFIT_ADAPTER_REAL_BRIDGE_HYDRATED_CORE` / GROUP / Composite strict。
...
**CaseId（C-22 出库/核销 RealBridge 骨架）**：`PLANNER_EXECUTOR_STOCK_REDUCE_ADAPTER_REAL_BRIDGE_CORE` — **Removed（P1-B2a）**；GraphCase 已删，替代：`PLANNER_EXECUTOR_STOCK_REDUCE_ADAPTER_REAL_BRIDGE_HYDRATED_CORE` / GROUP / Composite strict。
...
**CaseId（C-16 采购 Adapter 骨架）**：`PLANNER_EXECUTOR_PURCHASE_ADAPTER_CORE` — **Removed（P1-B2a）**；`AiPlannerExecutorPurchaseAdapterGraphCase` 已删；替代：`PLANNER_EXECUTOR_PURCHASE_ADAPTER_FAKE_OK_CORE` / HYDRATED / GROUP / Composite strict。
...
**CaseId（C-7 营收 Adapter 半真实占位）**：`PLANNER_EXECUTOR_REVENUE_ADAPTER_CORE` — **Removed（P1-B2a）**；GraphCase 已删；替代：`PLANNER_EXECUTOR_REVENUE_ADAPTER_FAKE_OK_CORE` / HYDRATED / GROUP / Composite strict。详见 §16（历史）。
...
**CaseId（C-12 Real ReadBridge + `revenue_query`）**：`PLANNER_EXECUTOR_REVENUE_ADAPTER_REAL_BRIDGE_CORE` — **Removed（P1-B2a）**；GraphCase 已删；替代：`PLANNER_EXECUTOR_REVENUE_ADAPTER_REAL_BRIDGE_HYDRATED_CORE` / GROUP / Composite strict。详见 §21（历史）。
...
**CaseId（C-31 + C-31.1 经营诊断 Composite MOCK 骨架）**：`PLANNER_EXECUTOR_BUSINESS_DIAGNOSIS_COMPOSITE_CORE` — **Historical / Retired candidate（P1-B B1）**；六步全 MOCK；**`plannerCompositeHonesty=COMPOSITE_SKELETON_ONLY`**。strict 主验收已迁至 **C-35 / C-48 / C-42**。**B2** 可删 GraphCase 与 `AiPlannerExecutorMockGraphCase` 分支。

**C-32～C-34（Historical / Retired，P1-A）**：原分步 Composite caseId（`…_COMPOSITE_REVENUE_CORE` / `…_REVENUE_PURCHASE_CORE` / `…_REVENUE_PURCHASE_STOCK_CORE`）及对应 Hybrid 执行器、GraphCase 已摘除；**请用 C-35 ALL_REAL / C-48 GROUP / C-42 STOCK_DEGRADED**。`stepId` 见 **`CompositeBusinessDiagnosisStepIds`**。
...
| `plannerCompositeHonesty` | **`PLANNER_EXECUTOR_BUSINESS_DIAGNOSIS_COMPOSITE_CORE`**：**`COMPOSITE_SKELETON_ONLY`**（**Historical / Retired candidate，P1-B B1**）；**C-32～C-34**：**Retired（P1-A）**；**C-35** **`…_ALL_REAL_CORE`**：**`COMPOSITE_ALL_DATA_REAL_DIAGNOSIS_MOCK`**（**主验收**）；**C-48** **`…_GROUP_CORE`**：**`COMPOSITE_GROUP_ALL_DATA_REAL_DIAGNOSIS_DETERMINISTIC`**（**主验收**）；**C-42** **`…_STOCK_DEGRADED_CORE`**：**`COMPOSITE_STOCK_DEGRADED_DIAGNOSIS_DETERMINISTIC`**（**主验收**） |
| `plannerCompositeNote` | **C-31**：**`skeleton only; real hydrated adapters not invoked`**；**C-32～C-34**：**Retired（见 composite-plan-design §2.1～2.3）**；**C-35**：**`revenue, purchase, stock_reduce and dish_profit real hydrated adapters invoked; diagnosis/recommendation remain mock`**；**C-48**：**`group composite; four group hydrated adapters invoked; diagnosis deterministic; recommendation mock`** |
...
与 `PlannerStep.targetTool` / Agent 命名对齐的约定键（**文档表格 only**，无对应 Java 常量类）；线上 Tool 常量以 `AiBusinessToolIds` 为准，差异在表中注明。
...
**后续接真实 Agent**：实现 `PlannerStepExecutor`（或分层 Facade），内部按 `targetTool` / §14.4 文档键名分派到现有 `Business*Agent`、Tool Bean 或 Graph 节点；`PlannerExecutor` 仅 `new PlannerExecutor(PlannerExecutorExecutionMode.ADAPTER, realExecutor)`。**禁止**在 `PlannerExecutor` 内 `import` Domain Agent 实现类。
...
- **键源**：与计划模板一致（参见 §14.4 文档表格及 `BusinessAgentNames` / `AiBusinessToolIds` 对照）；实现类可用常量相等判断，**禁止**对用户原文做 contains/regex 作为路由依据。
...
### 21.4 Harness：`PLANNER_EXECUTOR_REVENUE_ADAPTER_REAL_BRIDGE_CORE`（Removed P1-B2a）

- 原 **`AiPlannerExecutorRevenueAdapterRealBridgeGraphCase`** 已删；见 **`PLANNER_EXECUTOR_REVENUE_ADAPTER_REAL_BRIDGE_HYDRATED_CORE`** / GROUP / Composite strict。
- （历史）原 GraphCase 行为：计划在 `revenueExecutionContext` 上挂不完整上下文（**故意** `runState==null`、`resolvedQueryContext==null`，`revenueReadRequest` 只为切片），使默认 curl 摘要诚实为 `ADAPTER_NO_RUN_STATE` / `run_state_ref_not_hydrated`；摘要字段 `plannerRevenueAdapterHonesty=REAL_BRIDGE_HARNESS_INCOMPLETE_CONTEXT`。
...
- **详设**：[`docs/ai/purchase-planner-adapter-design.md`](./purchase-planner-adapter-design.md) — **`purchase_overview`**、`PurchaseAgent` + `PurchaseOverviewToolExecutor` + `PurchaseToolRequestContext` + `PurchaseAnswerPlan`、**STORE 单店 Hydrated（§11 字段权威；§12 C-19 curl 收口 + 限制 + 后续域模板）**、禁止项、caseId：（**Historical removed**：`purchase_query` 已删）
  - `PLANNER_EXECUTOR_PURCHASE_ADAPTER_FAKE_OK_CORE`（**`…_CORE` / `…_REAL_BRIDGE_CORE` Removed P1-B2a**）
...
| caseId | 状态 |
|--------|------|
| `PLANNER_EXECUTOR_STOCK_REDUCE_ADAPTER_CORE` | **Removed（P1-B2a）** |
| `PLANNER_EXECUTOR_STOCK_REDUCE_ADAPTER_FAKE_OK_CORE` | **C-21** 已注册（本轮不动） |
| `PLANNER_EXECUTOR_STOCK_REDUCE_ADAPTER_REAL_BRIDGE_CORE` | **Removed（P1-B2a）** |
...
**C-27（RealBridge 骨架，已落地）**：**`DishProfitPlannerRealReadBridge`**（`readWithExecutionContext`；上下文缺失 → **`ADAPTER_NO_RUN_STATE` / `ADAPTER_NO_RESOLVED_CONTEXT`**；**`new`** 无 Bean → **`DISH_PROFIT_REAL_READ_BRIDGE_SKELETON`**）。**Harness**：**`PLANNER_EXECUTOR_DISH_PROFIT_ADAPTER_REAL_BRIDGE_CORE`** **Removed（P1-B2a）**；见 HYDRATED / GROUP / Composite strict。
...
**C-41 阶段总收口（仅文档）**：**C-30～C-40** 路线表、最终链路、**ALL_REAL_CORE** curl 观测、限制、原则与 **C-42～C-46** 见 **本文 §27**。
...
- [`docs/ai/business-diagnosis-composer-readonly-design.md`](./business-diagnosis-composer-readonly-design.md) — **C-51** **`BusinessDiagnosisCompositeReadonlyComposer`**（已实装）+ Gate 指针
...
- [`docs/ai/business-diagnosis-shadow-rollout-plan.md`](./business-diagnosis-shadow-rollout-plan.md) — **C-64** **`SHADOW` 灰度上线策略**（运营手册；**ShadowPolicy** 已实装）：范围、限流、指标、§5 关闸；**§7** → **C-66+**（暂缓）
...
- [`TODO_MULTI_AGENT.md`](../TODO_MULTI_AGENT.md) — 多智能体实现进度与阶段收口
