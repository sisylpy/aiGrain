...
| caseId | 类 | 预期行为 |
|--------|-----|----------|
| `PLANNER_EXECUTOR_STOCK_REDUCE_ADAPTER_CORE` | `AiPlannerExecutorStockReduceAdapterGraphCase` | 无 `StockReducePlannerReadBridge` → 首步 `DEGRADED`；`plannerStockReduceAdapterHonesty=ADAPTER_NO_REAL_CONTEXT`；`plannerExecutorTrace.degradedSteps` 含 `step_stock_reduce_adapter` |
...
| `PLANNER_EXECUTOR_STOCK_REDUCE_ADAPTER_REAL_BRIDGE_CORE` | `AiPlannerExecutorStockReduceAdapterRealBridgeGraphCase` | `StockReducePlannerRealReadBridge`（Harness `new`）；计划**不Hydrate** `AiRunState`/`AiResolvedQueryContext` → 首步 `DEGRADED`（如 `ADAPTER_NO_RUN_STATE:run_state_ref_not_hydrated`）；**不**调用 `StockReduceQueryToolExecutor`；`plannerStockReduceAdapterHonesty=REAL_BRIDGE_HARNESS_INCOMPLETE_CONTEXT` |
