...
| `dish_profit_analysis` | **`VIEW_DISH_SALES`** 且 **`VIEW_COST`**（专用 **`evaluateDishProfitAnalysisInvocation`**，非 `requiredPermissionForTool` OR 语义）。**D-8** `dish_sales_query_path` 与 **成本链**（`cost_diagnosis_path`）第 4 步均执行本品；标价收入读 **`businessInsightSummary.totalActualRevenue`**。另有 **角色拒答**：采购类（**`CostInsightIntentConvergence#isProcurementCostConvergenceRole`**）→ **`forDishProfitPurchaserDenied`**；**`WAREHOUSE_MANAGER` / `REGION_WAREHOUSE`** → **`forDishProfitWarehouseDenied`**；**`DELIVERY_SUPPLIER` / `DELIVERY_DRIVER` / `COUPON_OPERATOR`** → **`forDishProfitUnsupportedRoleDenied`** |
