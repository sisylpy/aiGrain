#!/usr/bin/env bash
# 采购主线 + 供货/商品锚 execution 分层回归（7 case，约数分钟级，替代频繁全量 bundle）
# 用法:
...
run_case_payload "PURCHASE_SUPPLIER_RANKING_ANCHOR_EXECUTION_GOODS_UNIT_PRICE_3" \
  "PURCHASE_SUPPLIER_RANKING_ANCHOR_EXECUTION_GOODS_UNIT_PRICE_3" \
  "这个月哪个供应商供货金额最高" \
  "上个月呢" \
  "采购了哪些商品？单价分别是多少？"

run_case_payload "PURCHASE_GOODS_RANKING_ANCHOR_EXECUTION_SUPPLIER_UNIT_PRICE_2" \
